...
# that enable *.DOMAIN domains to resolve to the server IP address locally.
...
echo ""
echo "DNS rewrites configured for local domain resolution:"
echo "  - glance.$DOMAIN      → $SERVER_IP"
echo "  - grafana.$DOMAIN     → $SERVER_IP"
echo "  - hortusfox.$DOMAIN   → $SERVER_IP"
echo "  - traefik.$DOMAIN     → $SERVER_IP"
echo "  - n8n.$DOMAIN         → $SERVER_IP"
echo "  - bookwyrm.$DOMAIN    → $SERVER_IP"
echo "  - ollama.$DOMAIN      → $SERVER_IP"
echo "  - prometheus.$DOMAIN  → $SERVER_IP"
echo "  - alerts.$DOMAIN      → $SERVER_IP"
echo "  - adguard.$DOMAIN     → $SERVER_IP"
echo ""
echo "Note: AdGuard will resolve *.$DOMAIN to your local server IP ($SERVER_IP)"
echo "      This overrides public DNS for local network access."
echo ""
echo "Next steps:"
echo "  1. Restart AdGuard: docker compose restart adguard"
echo "  2. Test DNS resolution: dig @127.0.0.1 glance.$DOMAIN +short"
