...
2. **AdGuard Home** provides DNS resolution for `*.${DOMAIN}` domains
...
1. Open browser
2. Navigate to `https://glance.${DOMAIN}` (or any service)
...
```bash
# Test HTTP redirect
curl -I http://servicename.${DOMAIN}

# Test HTTPS access
curl -Ik https://servicename.${DOMAIN}
```
...
- **Port:** 11434
- **Authentication:** None
- **API Docs:** https://ollama.${DOMAIN}/api
...
| n8n | https://n8n.${DOMAIN} | https://IP:5678 |
| Glance | https://glance.${DOMAIN} | http://IP:8282 |
| ... | ... | ... |
```
...
# All .${DOMAIN} domains should use https://
