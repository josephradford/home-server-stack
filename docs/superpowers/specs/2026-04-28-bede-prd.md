...
| usage-collect.sh | dotfiles repo | Bash | Mac data collection pipeline | launchd |
| obsidian-git-backup.sh | dotfiles repo | Bash | Bidirectional vault git sync | launchd |
| claude-sessions.py | dotfiles repo | Python | Claude Code session summarisation | called by usage-collect |
| biome-iphone-screentime.py | dotfiles repo | Python | iPhone screen time from Biome files | called by usage-collect |
...
- `POST /ingest/vault` — receives CSV-in-JSON payloads from the Mac's `usage-collect.sh`. Parses screen time (Mac + iPhone), Safari history, YouTube history, podcasts, Claude sessions, and Bede sessions. Full daily replacement for screen time and Safari data. Bearer token authentication.
